          ...
          {tab === 2 && (
            <Stack spacing={2.5}>
              <Paper elevation={0} sx={{ p: 3, borderRadius: 2 }}>
                ...
                {resumes.length === 0 ? <Alert severity="info">选择职位并生成后，定制简历版本会保存在这里。</Alert> : resumes.map((resume) => <Paper key={resume.id} elevation={0} sx={{ p: 3, borderRadius: 2 }}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={1}><Box><Typography variant="h6">{resume.title}</Typography><Typography variant="body2" color="text.secondary">版本 {resume.schema_version} · {new Date(resume.created_at).toLocaleString()}</Typography></Box><Stack direction="row" spacing={0.5}><Button size="small" startIcon={<EditRoundedIcon />} onClick={() => navigate(`/resume-optimizer?resumeId=${resume.id}`)}>A4 编辑并保存</Button><Button size="small" startIcon={<VisibilityRoundedIcon />} onClick={() => setPreviewResumeId(String(resume.id))}>预览与导出</Button><Tooltip title="删除此版本"><IconButton color="error" aria-label="删除定制简历" onClick={() => deleteResume(resume)} disabled={working}><DeleteOutlineRoundedIcon /></IconButton></Tooltip></Stack></Stack><Typography variant="subtitle2" sx={{ mt: 2, mb: 1 }}>匹配分析</Typography><JsonPanel value={resume.match} /></Paper>)}
