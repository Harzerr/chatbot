from typing import List, Dict, Any


def format_chat_results(points) -> List[Dict[str, Any]]:
    """Helper method to format Qdrant points into chat message objects.

    Args:
        points: List of Qdrant points from a scroll query

    Returns:
        List of formatted chat message objects
    """
    results = []
    for point in points:
        payload = point.payload
        content = payload.get("page_content", "")
        metadata = payload.get("metadata", "")

        user_msg = ""
        assistant_msg = ""
        if "User:" in content and "Assistant:" in content:
            parts = content.split("Assistant:")
            user_part = parts[0].strip()
            assistant_msg = parts[1].strip() if len(parts) > 1 else ""
            user_msg = user_part.replace("User:", "").strip()

        chat_msg = {
            "id": str(point.id),
            "user_message": user_msg,
            "assistant_message": assistant_msg,
            "timestamp": metadata.get("timestamp", ""),
            "chat_id": metadata.get("chat_id", ""),
            "user_id": metadata.get("user_id", ""),
            "interview_role": metadata.get("interview_role"),
            "interview_level": metadata.get("interview_level"),
            "interview_type": metadata.get("interview_type"),
            "target_company": metadata.get("target_company"),
            "jd_content": metadata.get("jd_content"),
            "resume_content": metadata.get("resume_content"),
            "code_execution": metadata.get("code_execution"),
            "knowledge_context": metadata.get("knowledge_context"),
            "evidence_pack": metadata.get("evidence_pack"),
            "evaluation": metadata.get("evaluation"),
            "evaluation_status": metadata.get("evaluation_status"),
            "evaluation_job_id": metadata.get("evaluation_job_id"),
            "evaluation_error": metadata.get("evaluation_error"),
            "answer_counted": metadata.get("answer_counted"),
            "interview_phase": metadata.get("interview_phase"),
            "question_mode": metadata.get("question_mode"),
            "follow_up_count": metadata.get("follow_up_count", 0),
            "max_follow_ups": metadata.get("max_follow_ups", 0),
            "question_grounded": bool(metadata.get("question_grounded", False)),
            "question_grounding_version": metadata.get("question_grounding_version"),
            "question_evidence_ids": metadata.get("question_evidence_ids") or [],
            "question_evidence_items": metadata.get("question_evidence_items") or [],
            "evidence_feedback": metadata.get("evidence_feedback") or [],
        }
        if getattr(point, "score", None) is not None:
            chat_msg["_score"] = float(point.score)
        results.append(chat_msg)

    return results
